...
class OneShotDataPreparation():
    ...
    def _prepare_dataset(features_df):
        ...
        features_encoded_df, le = _data_conversion(features_df, False, le)

        # target_le = sklearn.preprocessing.LabelEncoder()
        # target_df = _data_conversion(target_df, True, target_le)

        return features_encoded_df.values#, target_df

    @staticmethod
    def _normalize(encoded_df):
        return pd.DataFrame(preprocessing.normalize(preprocessing.scale(encoded_df, axis=0), norm='max'))
