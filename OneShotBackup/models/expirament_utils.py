...
from features.OneShotDynamicFeaturesGenerator import OneShotDynamicFeaturesGenerator
...
def create_pipelines(seed, verbose=1):
    ...
    features = [('OneShotFeatures', OneShotDynamicFeaturesGenerator(generateAratios=True))]
    ...
    compressions = [('PCA', PCA(n_components=4)),
                 ]
    # Create pipelines
    pipelines = []
    ...
    for model in models:
        ...
        # Append model+features
        for feature in features:
            model_name = feature[0] + "_" + model_name
            pipelines.append((model_name, Pipeline([feature ,model])))

        # Append model+scaler
        for scalar in scalers:
            model_name = scalar[0] + "_" + model_name
            pipelines.append((model_name, Pipeline([scalar, model])))

        # To easier distinguish between with and without Additions (i.e: PCA)
        # Append model+compression
        for compression in compressions:
            model_name = compression[0] + "_" + model_name
            pipelines.append((model_name, Pipeline([compression, model])))

        # Append model+scaler+compression
        for scalar in scalers:
            for compression in compressions:
                model_name = scalar[0] + "_" + compression[0] + "_" + model_name
                pipelines.append((model_name, Pipeline([scalar, compression, model])))

        # Append model+scaler+features
        for feature in features:
            for scalar in scalers:
                model_name = feature[0] + "_" + scalar[0] + "_" + model_name
                pipelines.append((model_name, Pipeline([feature, scalers, model])))

        # Append model+scaler+compression+ features
        for feature in features:
            for scalar in scalers:
                for compression in compressions:
                    model_name = feature[0] + "_" + scalar[0] + "_" + compression[0] + "_" + model_name
                    pipelines.append((model_name, Pipeline([feature, scalar, compression, model])))
...
def run_cv_and_test_hypertuned_params(X_train, y_train, X_test, y_test, pipelines, scoring, seed, num_folds,
    ...
    for name, model in pipelines:
        ...
        elif scoring is "f_score":
            y_pred = model.predict(X_test)
            curr_test_score = f1_score(y_test, y_pred)
...
def get_hypertune_params():
    ...
    #OneShot features parameters
    oneshot_params = {'OneShotFeatures__generateAratios': [False, True],
                      'OneShotFeatures__generateActions': [False, True],
                      'OneShotFeatures__generateBehavioral': [False, True],
                      'OneShotFeatures__generateGaps': [False, True]}
    ...
    hypertuned_params = {"RF": rf_params,
                         ...
                         "KNN": knn_params,
                         "CART": cart_params,
                         "OneShotFeatures": oneshot_params
                         }
